...
// --- Tools ---
const coreFiles = ["AGENTS.md", "SOUL.md", "USER.md", "HORIZONS.md", "CONCEPTS.md", "TOOLS.md", "IDENTITY.md", "MEMORY.md", "HEARTBEAT.md", "BOOTSTRAP.md", "RIBOSOME.json", "NOCICEPTION.md"];
const protectedFiles = new Set(coreFiles);
server.setRequestHandler(ListToolsRequestSchema, async () => {
    const instincts = await kernel.loadInstincts();
    const coreTools = Object.entries(instincts).map(([name, config]) => ({
        name,
...
const HANDLERS = {
    ...
    "miniclaw_introspect": async (args) => {
        ...
        const fmt = (obj) => Object.entries(obj).sort((a, b) => b[1] - a[1]).map(([k, v]) => `- ${k}: ${v}x`).join('\n');
        if (scope === "tools")
    ...
    "miniclaw_entity": async (args) => {
        ...
        const m = { remove: 'remove', link: 'link', set_sentiment: 'updateSentiment' };
        if (m[action])
